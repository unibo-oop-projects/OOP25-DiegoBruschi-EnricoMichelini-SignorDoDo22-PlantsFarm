package it.unibo.plantsfarm.controller;

import com.fasterxml.jackson.databind.cfg.ContextAttributes.Impl;

import it.unibo.plantsfarm.controller.gamepanel.ImplControllerGamePanel;
import it.unibo.plantsfarm.model.GameState;
import it.unibo.plantsfarm.model.plant.GardenModel;
import it.unibo.plantsfarm.model.player.api.AbstractPlayer;

public class GardenController {
    private GameState gameState;
    private ImplControllerGamePanel controllerGamePanel;
    private AbstractPlayer player;
    private GardenModel gardenModel;

    public GardenController (GameState gameState, ImplControllerGamePanel controllerGamePanel, AbstractPlayer player) {
        this.gameState = gameState;
        this.controllerGamePanel = controllerGamePanel;
        this.player = player;
        this.gardenModel = new GardenModel();
    }
}
